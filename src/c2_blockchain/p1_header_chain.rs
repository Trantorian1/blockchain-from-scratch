...
impl Header {
    ...
    fn verify_sub_chain(&self, chain: &[Header]) -> bool {
        chain
            .iter()
            .fold_while((true, self), |(_, parent), header| {
                let mut hasher = DefaultHasher::new();
                parent.hash(&mut hasher);

                let is_valid_hash = hasher.finish() == header.parent;
                let is_valid_height = header.height == parent.height + 1;

                if !is_valid_hash || !is_valid_height {
                    FoldWhile::Done((false, header))
                } else {
                    FoldWhile::Continue((true, header))
                }
            })
            .into_inner()
            .0
